...
- `sample_weight.py`：显式时间衰减权重（exponential、正半衰期天数、均值1归一化）；拒绝未来监督原点、未知参数和权重下溢。原唯一生产入口（红太阳年度脚本）已随 2026-09-25 场景收敛退役，当前无活动生产消费者，能力由合成测试保持；其他入口不能仅凭schema接受该字段就声称已经使用权重。
