...
> 后续职责收敛说明：本文原设计和历史实施记录保留，不回改历史承诺。当前实际结构以 `decomposition/README.md` 为准；新增的组件拟合、配置唯一入口、通用周期分析移包、诊断 IO 上移及旧产物拒绝策略，记录于 `.hermes/plans/2026-09-07_003942-decomposition-boundaries.md`。本次同时修复 STL/MSTL 非默认外推参数接线，因此不属于全量零行为变化迁移。
