...
稳定概率合同位于 `forecasting_core/{probabilistic_spec,artifacts}.py`；point/quantile forecaster 位于 `model_forecasting/predictor.py`。Canonical runtime 显式启用 CQR 时输出 `predict_pi*`，final 修正量随 bundle 保存并用于部署，不在部署期重校准。
