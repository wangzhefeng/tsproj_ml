...
class PredictionHelper:
    ...
    def prepare_features(self):
        ...
        logger.info(f"{self.log_prefix} Available exogenous features: {self.available_exogenous}")
        
        # history data length： Need enough history to form the maximum lag
        self.max_lag = max(self.args.lags) if self.args.lags else 1
        logger.info(f"{self.log_prefix} Max lag: {self.max_lag}")

        # 分块大小
        self.block_size = min(self.args.lags) if self.args.lags else 1
        logger.info(f"{self.log_prefix} Block size: {self.block_size}")
    ...
    def univariate_single_multi_step_direct_output_forecast(self):
        ...
        if not self.args.is_testing and self.args.is_forecasting:
            ...
            X_test_future_df = self.df_future
        
        # 特征预处理（预测模式）
        X_test_processed = self.feature_scaler.transform(X_test_future_df, categorical_features)
    ...
    def univariate_single_multi_step_direct_forecast(self):
        ...
        relevant_history_for_lags = self.df_history.iloc[-self.max_lag:].copy()
        # Use the first point of df_future_for_prediction for exogenous features
        forecast_start_point_exogenous = self.df_future.iloc[0:1].copy()
        ...
        # 特征预处理（预测模式）
        X_forecast_input_processed = self.feature_scaler.transform(X_forecast_input, categorical_features)
        self.feature_scaler.validate_features(X_forecast_input_processed, stage="prediction")
        
        # Expects (1, H) output -> [H] array
        Y_pred_multi_step = self.model.predict(X_forecast_input_processed)[0]
        
        # Assign predictions to df_future_for_prediction
        if len(Y_pred_multi_step) >= len(self.df_future):
            Y_pred = Y_pred_multi_step[:len(self.df_future)]
        else:
            logger.warning(f"Predicted {len(Y_pred_multi_step)} steps but df_future requires {len(self.df_future)} rows. Padding predictions.")
            Y_pred = np.pad(Y_pred_multi_step, (0, len(self.df_future) - len(Y_pred_multi_step)), 'edge')
    ...
    def univariate_single_multi_step_recursive_forecast(self):
        ...
        last_known_data = self.df_history.iloc[-self.max_lag:].copy()
        # Ensure target_feature is present in last_known_data
        if self.target_feature not in last_known_data.columns and self.target_feature in self.df_history.columns:
            last_known_data[self.target_feature] = self.df_history[self.target_feature].iloc[-self.max_lag:]
        ...
        for step in range(self.args.horizon):
            ...
            current_step_df = self.df_future.iloc[step:step+1].copy()
            ...
            # 5.特征预处理（预测模式）
            current_features_for_pred_processed = self.feature_scaler.transform(current_features_for_pred, categorical_features)
            self.feature_scaler.validate_features(current_features_for_pred_processed, stage="prediction")
            # 5.进行预测
            y_pred_step = self.model.predict(current_features_for_pred_processed)[0]
            y_preds.append(y_pred_step)
            # 6.将预测值更新回 df_history，以便为下一步预测提供滞后特征
            new_row_for_last_known = current_step_df.copy().iloc[-1:]
            new_row_for_last_known[self.target_feature] = y_pred_step # Add the prediction as the new 'actual' for lag calculation
            # 将新行添加到历史数据中，进行下一次循环
            last_known_data = pd.concat([last_known_data, new_row_for_last_known], ignore_index=True)
            last_known_data = last_known_data.iloc[-self.max_lag:]
    ...
    def univariate_single_multi_step_direct_recursive_forecast(self):
        ...
        # 分块递归预测
        num_blocks = int(np.ceil(self.args.horizon / self.block_size))
        ...
        for block_idx in range(num_blocks):
            ...
            for step in range(block_start, block_end):
                ...
                temp_df_featured, predictor_features, _, categorical_features_updated = self.create_features(
                    df_series=combined_df,
                    endogenous_features_with_target=[self.target_feature],  # 只使用目标变量！
                    exogenous_features=self.available_exogenous,
                    target_feature=self.target_feature,
                    categorical_features=self.categorical_features,
                )
                ...
                # 5. 特征缩放
                current_features_for_pred_processed = self.feature_scaler.transform(current_features_for_pred, self.categorical_features)
                self.feature_scaler.validate_features(current_features_for_pred_processed, stage="prediction")
                ...
                # 7. 更新历史数据（用于下一步预测）
                new_row_for_last_known = current_step_df.copy().iloc[-1:]
                new_row_for_last_known[self.target_feature] = y_pred_step
                
                last_known_data = pd.concat([last_known_data, new_row_for_last_known], ignore_index=True)
                last_known_data = last_known_data.iloc[-self.max_lag:]  # 只保留需要的历史长度
    ...
    def multivariate_single_multi_step_direct_forecast(self):
        ...
        for endo_feat in self.endogenous_features:
            if endo_feat not in relevant_history_for_lags.columns and endo_feat in self.df_history.columns:
                relevant_history_for_lags[endo_feat] = self.df_history[endo_feat].iloc[-self.max_lag:]
        ...
        combined_featured, predictor_features_for_forecast, _, categorical_features_updated = self.create_features(
            df_series=combined_for_features,
            endogenous_features_with_target=self.endogenous_features,  # 所有内生变量(包括目标变量)
            exogenous_features=self.available_exogenous,
            target_feature=self.target_feature,
            categorical_features=self.categorical_features
        )
        ...
        # 6. 特征缩放
        X_forecast_input_processed = self.feature_scaler.transform(X_forecast_input, self.categorical_features)
        self.feature_scaler.validate_features(X_forecast_input_processed, stage="prediction")
        ...
        if len(Y_pred_multi_step) >= len(self.df_future):
            ...
            logger.warning(
                ...
                f"but df_future requires {len(self.df_future)} rows. Padding predictions."
            )
            Y_pred = np.pad(Y_pred_multi_step, (0, len(self.df_future) - len(Y_pred_multi_step)), 'edge')
    ...
    def multivariate_single_multi_step_recursive_forecast(self):
        ...
        all_endogenous_original_cols = [col.replace("_shift_1", "") for col in self.target_output_features] # original names of predicted endogenous
        ...
        for col in all_endogenous_original_cols:
            if col not in last_known_data.columns and col in self.df_history.columns:
                last_known_data[col] = self.df_history[col].iloc[-self.max_lag:]
        ...
        for step in range(self.args.horizon):
            ...
            # 1. Prepare current features for prediction
            current_step_df = self.df_future.iloc[step:step+1].copy() # Future exogenous and datetime features for this step
            ...
            temp_df_featured, predictor_features, _, categorical_features = self.create_features(
                df_series=combined_df,
                endogenous_features_with_target=self.endogenous_features, # All actual endogenous cols including target
                exogenous_features=self.available_exogenous,
                target_feature=self.target_feature,
                categorical_features=categorical_features,
            )
            ...
            next_pred_values_array = self.model.predict(current_features_for_pred_processed)[0] # [0] because predict returns [[val1, val2, ...]]
            # Map predictions back to their shifted column names
            next_pred_dict = dict(zip(self.target_output_features, next_pred_values_array))
    ...
    def multivariate_single_multi_step_direct_recursive_forecast(self):
        ...
        # 确保所有内生变量都在历史数据中
        for endo_feat in self.endogenous_features:
            if endo_feat not in last_known_data.columns and endo_feat in self.df_history.columns:
                last_known_data[endo_feat] = self.df_history[endo_feat].iloc[-self.max_lag:]
        ...
        for block_idx in range(num_blocks):
            ...
            for step in range(block_start, block_end):
                ...
                # 6. 预测目标变量
                y_pred_target = self.model.predict(current_features_for_pred_processed)[0]
                ...
                # 更新目标变量的值（使用预测值）
                new_row_for_last_known[self.target_feature] = y_pred_target
