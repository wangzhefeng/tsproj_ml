...
本文档记录尚未解决的工程问题、技术债和配置体系优化事项。业务实验与场景研究事项继续记录在 [`docs/scenarios/aidc_load_15min_short/TODO_AIDC.md`](scenarios/aidc_load_15min_short/TODO_AIDC.md)（2026-09-25 场景收敛后从 `docs/` 迁入场景目录，后随文档统一迁入 docs/scenarios/）。
...
| OPT-020 | 已完成 | 2026-05 pred 13h × 12 列已插补，12 份场景资产重建；月频/日频代表生命周期已跨缺口通过 | 仅指数据修复与代表运行完成；strict 标记不证明严格 as-of，资格缺口见 OPT-022 |
| OPT-021 | 进行中 | 权威源、场景数据和 1092 份 future_path 已修复；月频/日频各一份代表配置及 15min 试点通过，仍待按需重建其余正式结果 | 重跑 identity 与当前配置一致；不回填旧身份目录；不把代表运行表述为全量结果 |
| OPT-022 | 待处理 | 核实离线天气插补的历史可得性、证据传播与部署限制；严格 as-of／实盘资格尚未验收 | 不用 strict 字段或测试通过替代来源证明；实施语义变更前单独确认 |

**本轮文档更新边界**：仅更新天气收尾状态，保留历史实施证据；按用户要求不跑测试、不重跑模型。OPT-020 的数据修复完成不等于 OPT-021 的全量结果重建完成，也不等于 OPT-022 的严格 as-of／实盘资格通过。下方历史性能任务执行顺序不构成本轮长跑授权。
...
- **状态**：已完成（2026-09-08；离线历史预报插补口径）
- **优先级**：P1
- **类型**：外部数据依赖 / 天气预报数据缺口
- **来源**：2026-09-07 统一天气资产迁移收尾汇报「未完成 / 如实 blocked」
- **涉及范围**：`aidc_power_month` 月/日频 2026-05 回测、`dataset/shared/weather/extracted/actual/weather_in_20250101_20260831.csv`、15min×3/ESS/power_month 日月频场景 history 数据

> 本项仅按“离线数据修复 + 代表生命周期运行”收口。以下初始事实、问题和建议保留当时快照；最新完成边界以本项末尾澄清为准，严格 as-of／实盘资格另见 OPT-022。
...
### 2026-09-08 增量：8 月补数后缺口仍未修复

用户 2026-09-08 补充 8 月数据（权威源扩展至 2026-08-31 23:00，SHA256 前 16 位 `7a9b746cc5842b58`，新增 408 行）。逐项复核：2026-05-13 19:00 → 05-14 07:00 的 pred 缺失**原样保留**（13 行 × 12 列 = 156 单元格仍缺），全表 pred 缺失行仍为 35 行（分布 2025-06-24/25、2025-07-16/17、2026-05-13/14 与补数前一致）。本条状态维持**阻塞**，待供应商 2026-05-13/14 历史预报快照。

### 2026-09-08 实施：用户授权同供应商 pred_ 插补，阻塞解除

用户随后授权直接填补权威源该时段全部 `pred_` 列。初始 `rt_` 实测替代建议未执行；为避免完美预报泄漏，改为只使用同一供应商 `pred_` 序列的变量分组混合插补：温度/气压/风速/云量用缺口两端线性插值，风向按圆周最短弧插值，辐射/相对湿度/降水用过去 7 天同小时中位数并施加物理边界。方法在 71 个完整同型历史窗口上做掩码比较后确定；不使用 Open-Meteo、不混源。

- `.hermes/plans/weather-fill-may-gap.py` 只修改目标 13 行，窗口外逐行字节相等断言通过；填入 156 个单元格。源 SHA256 由 `7a9b746cc5842b58...` 变为 `b809b87d463d757d...`，逐列方法、参考值与填充值见 `.hermes/plans/weather-may-gap-fill-report.json`。
- `scripts/build_scenario_weather.py` 重建 12 份场景资产；15min×3 各 52 行、ESS 156 行、日频 2 行、月频 1 行的修复窗口全部无 pred 缺失，六份 history 全时段模型消费 pred 列缺失计数均为 0。
- 月频 `enet_usmd_mean.yaml` 完整生命周期退出 0：3 fold 覆盖 2026-05-31→07-31，CV/08-31 预测均零 NaN，schema-2 bundle 重载 fingerprint `6e80334062b1` 一致。日频 `add_exogenous_weather_date/enet_usmd_mean.yaml` 退出 0：6 fold、CV 181 行明确包含 2026-05-13/14 且零 NaN，8 月预测 31 行零 NaN。
- 语义边界：这些值是**同供应商 pred_ 序列的离线历史缺失插补**，不得标记为供应商原始值。它只影响历史回测输入，不改变 final fit 使用的 `rt_` 或 2026-08 最终预测使用的原始 future `pred_`；活动配置和实际生成 bundle 的 `execution_mode` 回读均为 `strict`，不将其误称为 research bundle。
...
- [x] 权威源 2026-05-13 19:00 → 05-14 07:00 的 pred_ 12 列无缺失（用户明确裁决的同供应商 pred_ 插补口径已落地并留档）。
- [x] 月频回测跨 2026-05 fold 运行通过，评分/predict/final fit 产物完整且身份为当前合同 identity；schema-2 bundle 已重载核验。

### 2026-09-08 完成边界澄清（本轮仅更新文档）

- 数据缺失导致的运行阻塞已解除；插补不是恢复了供应商原始历史预报快照。未使用实测或异源数据，不足以证明没有未来信息泄漏：端点插值使用缺口后的值，同小时历史参考值也必须相对每个回测 origin 核实其实际发布/接收时间。
- 前述“execution_mode 为 strict”仅是产物字段回读事实，不能据此弱化研究限制或证明严格历史 as-of。最终 fit 使用 rt_、8 月预测输入未改，也不能替代对回测、校准、选模及部署证据的影响审计；该未完成工作转入 OPT-022。
- 既有执行证据（不是本轮新跑）：`env -u PYTHONPATH .venv/bin/python .hermes/plans/weather-fill-may-gap.py` 修复 156 个单元格；同一解释器运行 `scripts/build_scenario_weather.py` 产出 12 份文件，12/12 meta 已关联源 SHA 与修复报告。
- 既有代表运行命令：`env -u PYTHONPATH .venv/bin/python run.py --config-yaml config/aidc_power_month/route_A/freq_1month/window_length_10/enet_usmd_mean.yaml` 和 `env -u PYTHONPATH .venv/bin/python run.py --config-yaml config/aidc_power_month/route_A/freq_1day/add_exogenous_weather_date/enet_usmd_mean.yaml` 均退出 0；产物范围见上方实施记录，不外推到其余配置。
- 既有最终测试证据：`env -u PYTHONPATH .venv/bin/python tests/run_suite.py fast` 为 289 项通过；同一入口 `integration --match weather` 为 60 项通过，`integration --match inference_columns` 为 5 项通过。本轮未重跑这些命令。

---

## OPT-021：月频当前合同身份收口与正式结果目录重建

- **状态**：进行中（数据已补齐并完成代表运行，待按需重建其余正式结果）
- **优先级**：P2
- **类型**：运行任务 / 正式产物重建
- **来源**：2026-09-07 统一天气资产迁移收尾汇报「未完成 / 如实 blocked」
- **涉及范围**：`aidc_power_month` 月频与日频最终预测、15min×3 与 ESS 场景正式结果目录、`results/_compiled_features` 特征缓存

> 下方“从未产生正式结果”“results 为空”及“待 OPT-020 解除”均为此前阶段快照，不代表最新状态；最新状态见“OPT-020 已解除，代表结果已重建”和本项末尾断点。
...
### 2026-09-08 增量：8 月数据已补充，阻塞解除待执行

用户已补充 8 月数据并落位权威源（2026-09-08 14:33 修改）。程序化复核：

- **范围扩展**：2025-01-01 00:00 → **2026-08-31 23:00**（原止于 08-14 23:00），新增 408 行（8-15 00:00 → 8-31 23:00 完整小时网格）。
- **pred 覆盖**：8-15 之后 pred_ 12 列 × 408 行**零缺失** → 日频 2026-08 全月 31 天（origin 2026-07-31 + calendar_month horizon=31）与月频 2026-08-31 单行的推理需求均满足。
- **rt 缺失（未来段，按合同不消费）**：8 月 rt_sh 缺 423h（08-01~08-18 段）、rt_ssr 缺 6h（08-04 21:00~08-05 02:00 夜窗，由既定夜间-0 政策吸收）、rt_ps 缺 7h；聚合消费列 rt_tt2/rt_dt/rt_ws10 零缺失，cal_rh 派生不受影响；rt 缺失不阻断 future 段（只读 pred_）。
- **2026-05 缺口未变**：见 OPT-020，月频回测仍受 2026-05 fold 阻塞。

### 2026-09-08 执行：文件名、场景数据与配置修复完成

1. **权威源改名**：`weather_in_20250101_20260814.csv` → `weather_in_20250101_20260831.csv`（内容字节不变，SHA256 前 16 位 `7a9b746cc5842b58`）。
2. **构建脚本**：`scripts/build_scenario_weather.py` FUTURE_END 扩至 2026-08-31 23:59（顺带修复历史双层 docstring 语法错误）；重建 12 份场景数据：15min future 2976 行（08-01~08-31）、ESS future 9792 行（07-29~08-31）、日频 future **31 行全月零 NaN**、月频 future **1 行（2026-08-31）零 NaN**——不再是 0 行头文件。history 各份不变（内容 SHA 与重建前一致）。旧 `*_20260814` future 文件（5 csv + 5 meta）已删；算力目录与 shared/weather/raw 保全不动。
3. **配置批量修复**（`.hermes/plans/weather-fix-future-refs.py` 退出 0，留档 `weather-future-refs-fix.json`）：1092 份配置 future_path 改为 20260831 系（15min×3=972、ESS=16、日频天气组=104）；日频 baseline 22 份无天气源不在修复面（构成核对闭合：972+104+56+16=1148 份在范围天气配置全部指向 20260831 系）。逐份 parse 零失败、悬空引用零。
4. **验证**：fast 289 OK；test_inference_columns+test_weather_config_migration 3 项 OK；`audit_weather_configs.py` 复跑 blocked 149 全部算力（范围外）、在范围零 blocker。**真实运行试点**：15min daily `enet_dirrec_weather.yaml` 全生命周期通过（identity `dirrec-enet-local-k1-560d5ec78f13`，回测 31 fold、预测 96 行、冷编译 47.6s）。月频/日频最终预测 8 月推理输入程序化验证完整（月频 2026-08-31 四映射列有值；日频 8 月 31 天零 NaN）。
5. **月频/日频全量 run 仍受 OPT-020 阻塞**：真实执行月频 run 在 calendar 回测 2026-05 fold RAISE（`inference columns contain missing values`，history 2026-05-31 月聚合行 pred 全 NaN——13h 缺口所致，合同预期行为）；日频回测同理将触及 2026-05-13/14 两天。**最终预测段（8 月）数据已就绪，待 OPT-020 解除后整链收口**。

**待执行（更新）**：OPT-020 解除（供应商补 2026-05-13/14 历史预报）后，重建场景数据并重跑月频/日频全部配置；其余场景按需重建正式结果。

### 2026-09-08 增量：OPT-020 已解除，代表结果已重建

- 2026-05 缺口按 OPT-020 记录的用户授权研究插补口径修复，12 份场景资产已重建；本条不再受 2026-05 数据缺口阻塞。
- 月频当前 identity `direct-enet-local-k1-6e80334062b1` 已完成 3 fold 回测、2026-08-31 最终预测、final fit 与 schema-2 bundle 重载；日频代表 identity `direct-enet-local-k1-21482359b555` 已完成 6 fold（含 05-13/14）及 8 月 31 天预测。
- 仍未重跑 1148 份天气配置的全部正式结果；其余场景按实际需要串行重建，避免 CPU 互抢。该剩余工作不影响“天气数据资产已修复”的结论。
...
1. 月频/日频：按上方「待执行」步骤重建 future 数据并跑完整生命周期（回测 + 最终预测 + final fit + bundle 重载）。
...
- [x] 月频当前身份 `6e80334062b1` 完成回测 + 最终预测 + final fit + bundle 重载（3 fold CV 零 NaN、2026-08-31 预测 1 行零 NaN、bundle fingerprint/dimensions 核验通过）。
- [ ] 需要结果的场景按需重建正式产物，回读核验产物 schema、身份与既有验收口径一致。

### 2026-09-08 当前剩余工作与下一断点

- 仍未全量重跑 1148 份天气配置；不因此自动启动全量任务。后续先确定实际需要结果的配置清单，再核对当前 identity、源数据版本与已有产物，避免重复跑或误用修复前证据。
- 数据文件修复、代表结果落盘、全量结果重建是三个不同完成层级。月频 bundle 已重载核验；日频此轮证据为运行与 CSV/model 文件回读，不将其扩写成未执行的部署预测一致性验证。
- 融合采用既有计划 §15.5 的单模型等价证据路径，不新增 weather × ensemble 配置；这只覆盖共享装载合同及融合机制兼容，不证明具体融合组合的收益。
- 下一断点：OPT-022 证据边界核实，以及获准范围内的 OPT-021 结果清单盘点。本轮不启动测试、模型运行或部署。

---

## OPT-022：补齐离线天气插补的严格 as-of 证据与部署资格门禁

- **状态**：待处理
- **优先级**：P1
- **类型**：信息可得性 / 回测可信度 / 产物资格
- **涉及范围**：天气插补报告、场景 meta、file 两段制与 `inference_columns`、回测/校准/选模证据、bundle 持久化和部署入口

### 当前事实与影响

1. OPT-020 已完成离线缺失填补，但填充值不是供应商原始快照；本次没有取得足以证明参考值在各回测 origin 已可得的发布/接收证据。
2. 既有回读发现月频 bundle 的 `execution_mode='strict'`，场景 meta 已关联 `offline_imputed_historical_forecast` 修复记录。标签和 sidecar 存在并不证明限制已传播到 bundle 或部署入口；本轮没有验证该通路。
3. 缺口后端点与同小时参考序列可能包含相对回测 origin 的事后信息。插补掩码误差较小、模型运行成功、通用天气测试通过，都不能消除该证据缺口。
4. 已跑 point 代表的 final fit/8 月输入与历史插补输入分离，不足以外推全部配置：校准、选模、融合及评估证据可能消费受影响回测结果，具体传播范围尚待核实。

### 建议方向

1. 按实际回测 origin 追踪插补参考值、源版本、真实发布/接收事实及所有消费者；无法证明的部分明确记为未验证，不补造 available_at，不通过缩窗回避。
2. 核实修复记录从源文件、场景 meta 到回测证据、bundle 和默认部署入口的传播。严格 as-of 与研究回放分开；需要研究资格限制时必须实际执行默认部署拒绝，不能只改文档或标签。
3. 先提交合同与影响范围供用户确认，再实施必要代码/配置调整。不得仅因已有 bundle 写着 strict 就认定其有实盘资格，也不得擅自放宽既有研究限制。

### 风险

- 不当沿用 strict 标记可能让含事后信息的回测被用于生产决策；反过来，未经影响追踪就把所有产物一概降级也缺少证据。
- 资格合同调整可能改变 fingerprint、缓存、校准及产物兼容性；不自动删除或重跑已有结果。

### 验收标准

- [ ] 各受影响回测 origin 的参考数据可得性有真实证据，或明确隔离为非严格 as-of 的研究回放。
- [ ] 修复来源与资格限制沿实际消费者传播，覆盖回测、校准/选模及必要的融合链路。
- [ ] bundle 标记与真实证据一致；研究 bundle 默认生产部署被拒绝，并有对应实际验证。
- [ ] 完成影响清单、用户合同裁决及必要定向验证，保留命令和结果；未验收前不宣称实盘资格通过。

**本轮记录**：仅登记缺口与下一断点，未执行测试、代码修复、资格改写或部署。
