...
`weather_generator/` 已建成（按合同、适配、资产、气象派生、重采样、情景、请求编排分文件），当前保留为研究回放/取证链、非活动链，合同细节见 [`weather_generator.md`](weather_generator.md)；活动配置采用 file + `inference_columns`，不引用本子包。
