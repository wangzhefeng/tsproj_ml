...
- 本地入口只接受 canonical 单模型 schema YAML：运行 `env -u PYTHONPATH .venv/bin/python main.py --config-yaml <path>`。必须显式指定配置，无硬编码默认值；非 canonical YAML 一律 RAISE。引用式 Ensemble 使用 `run.py`。
...
- 运行与测试统一使用根目录 `.venv/` 虚拟环境（`.venv/bin/python`），依赖唯一来源为 `pyproject.toml` + `uv.lock`，依赖变更使用 `uv add`。测试直接调用 `.venv/bin/python`，不调用 uv、不设置 `UV_CACHE_DIR`、不依赖或重建 `.uv_cache/`；缓存删除不代表更换虚拟环境或依赖管理工具
...
- **`tests/` 已纳入版本控制**（2026-08-15 起移出 `.gitignore`）：改动被测模块时必须同步修复测试导入与接口断言。常规修改运行 `env -u PYTHONPATH .venv/bin/python tests/run_suite.py fast` 并补相关定向测试；`run_suite.py` 校验解释器必须来自项目 `.venv/`，其他环境直接拒绝。`integration` 为真实后端/运行链，`audit` 为全仓配置/资产及场景数据链，收口使用 `all`（与原生 `unittest discover -s tests -p "test_*.py"` 同一全集）。新测试默认纳入 integration，不得通过遗漏发现或 skip 提速；执行及覆盖映射见 `tests/README.md`。预测架构的最新精确计数只在 `docs/multistep_forecasting_redesign.md` 维护。AIDC 专项脚本目录路径含日期段、不是合法 Python 包，相关测试用 `sys.path.insert` 引导后按模块名导入。
