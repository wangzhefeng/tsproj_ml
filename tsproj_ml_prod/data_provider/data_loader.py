...
class DataLoader:
    ...
    def load_data(self, input_data: Optional[Dict] = None) -> Dict:
        """
        加载所有必要的数据
        ...
        """
        logger.info(f"{self.log_prefix} Loading data from production input_data")
        if input_data is None:
            raise ValueError(f"{self.log_prefix} input_data is required in production mode.")
        ...
        # 加载目标时间序列数据
        # ------------------------------
        df_target = input_data.get("df_power")
        if df_target is not None:
            loaded_data["target_series"] = df_target
            ...
            logger.error(f"{self.log_prefix} Target data `df_power` not found in input_data")
            raise ValueError(f"{self.log_prefix} Target data `df_power` not found in input_data")
        # ------------------------------
        # 加载日期类型数据
        # ------------------------------
        # 加载历史日期类型数据
        df_date_history = input_data.get("df_date")
        if df_date_history is not None:
            loaded_data["date_history"] = df_date_history
        ...
        # 加载未来日期类型数据
        df_date_future = input_data.get("df_date_future")
        if df_date_future is not None:
            loaded_data["date_future"] = df_date_future
        ...
        elif df_date_history is not None:
            df_date_all = df_date_history
        elif df_date_future is not None:
            df_date_all = df_date_future
        else:
            df_date_all = None
        # 数据收集
        loaded_data["date_history"] = df_date_all
        loaded_data["date_future"] = df_date_all
        # ------------------------------
        # 加载气象数据
        # ------------------------------
        # 加载历史气象数据
        df_weather_history = input_data.get("df_weather")
        if df_weather_history is not None:
            loaded_data["weather_history"] = df_weather_history
        ...
        # 加载未来气象数据
        df_weather_future = input_data.get("df_weather_future")
        if df_weather_future is not None:
            loaded_data["weather_future"] = df_weather_future
        ...
        elif df_weather_history is not None:
            df_weather_all = df_weather_history
        elif df_weather_future is not None:
            df_weather_all = df_weather_future
        else:
            df_weather_all = None
        # 数据收集
        loaded_data["weather_history"] = df_weather_all
        loaded_data["weather_future"] = df_weather_all
