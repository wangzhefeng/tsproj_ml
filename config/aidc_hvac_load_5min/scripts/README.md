...
scripts/
  ...
  v3/
    ...
    forecast_data/build_model_configs.py # 四组A1_all/v3配置，独立发布
```
...
- 四组配置为 `<组>/A1_all/v3/<设备口径>/lgbm_<方法>.yaml`，保持各组消融语义、九方法、14天训练及1天预测；仅历史回测，不启动正式训练或覆盖结果。

v3构建命令见[场景README](../README.md#a1_allv3固定窗口两路合计)。旧三楼填补、自动选窗和旧可视化CLI仅接受data_v1/v2，不把这些合同套到v3；公共天气适配支持data_v3。
