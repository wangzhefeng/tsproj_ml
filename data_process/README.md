...
`data_process/` 是进模型前的离线数据准备工具库：频率聚合、填补方法回测、异常分析、事件检测、峰谷和周期分析。周期检测的纯数值核心在 `ts_kernels/periods.py`；`periodicity_analysis.py` 承载 DataFrame 排序/有效样本选择、时间单位换算、规格、可视化、报告和 CLI。FFT top-k（`fft_top_k`）与 Engle-Granger 协整诊断（`coint_col`）为可选报告项；协整检验留在本离线工具中。
...
- **场景入口**：原 `config/aidc_load_15min_daily/load_event_analysis.py` 与 `config/aidc_load_month/load_event_analysis.py` 已随 2026-09-25 场景收敛退役（Git 溯源）；检测核心与产物合同不变，新场景入口待数据准备重构时重建。产物在 `dataset/<场景>/event_label_features/`（labeled_features.csv / events.csv / overview.png / report.md）。
