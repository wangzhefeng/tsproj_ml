...
- **消融矩阵的 no-op control**：矩阵中不消费目标特征的模型（SeasonalTemplate `st` 不消费 custom/load-state/weather 特征、USBR 不接入天气特征）**不混入实验组目录**，对照统一走 baseline 目录的同名配置（2026-08-25 起全场景一致，power_month weather_date 组的 `-control-no-weather` 配置已删除）；历史结果的 `-control-no-<feature>` 后缀（如 `-decomp-linear-control-no-state`）为旧口径，读数时按 baseline 同名配置对照
