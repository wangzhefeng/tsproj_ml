...
所有 C0–C3 配置均使用 USMDP point、`predict_steps: 288`、`detrend_target: true`，通过不同 `setting_suffix` 隔离结果目录。
...
策略特征专项测试为 `tests/test_ess_strategy_*.py`，覆盖状态编码、双时间轴、周期画像、相似日以及流水线泄漏/落盘契约。
