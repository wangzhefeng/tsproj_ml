...
MAPE / MAPE Accuracy 计算与预测图历史上下文掩码共用本模块的
``build_eval_mask``，避免历史上两处各写一份分位数逻辑而分叉。
...
def build_eval_mask(
    ...
    *,
    ...
    Returns:
        ...
        excluded_ratio)，``threshold`` 为下限（兼容原 ``_build_relative_mape_mask`` /
        ``_build_plot_mask`` 输出），``upper_threshold`` 为上限。
    """
