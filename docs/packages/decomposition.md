...
## 子包职责明细（原子包 README 并入）

- `configuration`：分解配置校验、规格和固定组件配方；不导入具体算法或构造器。
- `construction`：Registry 构造 Pipeline，component_factory 构造具体组件。__init__.py 不自动导入 Registry，避免经 Pipeline 回到本包时发生循环导入。
- `contracts`：历史分解/外推接口、分量数据类型以及时间轴与有限目标合同；不依赖配置、具体算法或 Pipeline。
- `extraction`：历史目标分解算法。`polynomial.py` 提取多项式历史趋势；`seasonal.py` 提取 STL/MSTL 原始趋势与各周期季节分量。仅返回 `ComponentFrame`，不拟合未来外推参数、不写文件。
- `extrapolation`：分量拟合与未来外推。`trend.py` 拟合多项式趋势及 damped 近期斜率，`seasonal.py` 从真实历史季节分量拟合相位模板。所有参数由 preset 显式传入，不接收 extractor 对象或动态注入的拟合状态。
- `composition`：加性目标恢复实现；输入残差预测与结构分量，不拟合算法，不做生命周期编排。
- `orchestration`：单序列分解生命周期编排；调用组件工厂和组件接口，不重新实现分解或外推算法。
- `diagnostics`：真实历史分量统计与残差频谱/跨窗口稳定性计算；不读取 Pipeline 私有属性，不执行文件 IO。
