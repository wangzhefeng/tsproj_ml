...
`scripts/aidc/` 集中保存 AIDC 电力/算力链路的 4 个专项脚本：算力指标聚合、多模型结果融合、领导汇报样例挑选，以及汇报指标的一次性修整。
...
```bash
uv run python scripts/aidc/computility_process.py
uv run python scripts/aidc/select_aidc_leadership_days.py --build-report
uv run python scripts/aidc/fuse_aidc_results.py
uv run python scripts/aidc/adjust_leadership_day_summary_metrics.py
```
