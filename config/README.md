...
- Direct 结果前缀沿用 direct、direct-pointwise、direct-pointwise-horizon 三类；cyclical 属于第三类内部特征变体，仅在元数据记录并由 fingerprint 区分，不从 pointwise 文件名推断语义。统一规则见 `model_forecasting/README.md`，修改展示前缀不改变 fingerprint、不自动迁移旧目录。
