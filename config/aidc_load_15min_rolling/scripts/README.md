# aidc_load_15min_rolling 场景天气适配
...
输入默认 `dataset/shared/weather/processed/weather_hourly.csv`，可用 `--processed <CSV>` 指定；要求同名 `.meta.json` 并核对CSV哈希。输出：`dataset/aidc_load_15min_rolling/weather_history_15min_20250101_20260831.csv`，同名metadata记录场景入口及共享资产的CSV/metadata哈希。重复运行重写派生产物，不删除其他文件；从任意工作目录用绝对脚本路径运行均可。
